...
class YoloLoss(nn.Module):
    ...
    def get_class_prediction_loss(self, classes_pred, classes_target, has_object_map):
        ...
        # Your code here
        loss = F.mse_loss(
            classes_pred[has_object_map], classes_target[has_object_map], reduction='sum')
        # loss = F.binary_cross_entropy_with_logits(classes_pred[has_object_map], classes_target[has_object_map], reduction='sum')
        return loss

    def get_no_object_loss(self, pred_boxes_list, has_object_map):
        """
        Parameters:
        B 是每个 grid cell 预测的边界框数量，5 是边界框的信息（x, y, w, h, confidence）
        ...
        for pred_boxes in pred_boxes_list:
            # for pred_boxes in pred_boxes_list[1:]: //使用此行的迴圈，產生的loss會相對正確的loss較低，所以box的confidence普遍偏高，採用的box也會多
            # Extract the confidence predictions from the prediction boxes
            pred_confidence = pred_boxes[..., 4]

            # Compute MSE loss only for cells without objects
            loss += F.mse_loss(pred_confidence[~has_object_map], torch.zeros_like(
                pred_confidence[~has_object_map]), reduction='sum')
    ...
    def get_contain_conf_loss(self, box_pred_conf, box_target_conf):
        """
        Parameters:
        预测的置信度值，大小为(-1, 1)，其中-1表示根据输入大小自动推断。
    ...
    def get_regression_loss(self, box_pred_response, box_target_response):
        ...
        reg_loss = F.mse_loss(
            box_pred_response, box_target_response, reduction='sum')
        return reg_loss
    ...
    def forward(self, pred_tensor, target_boxes, target_cls, has_object_map):
        ...
        pred_boxes_list = [pred_tensor[:, :, :, b *
                                       5:(b + 1) * 5] for b in range(self.B)]
        ...
        pred_boxes_vec = [pred_boxes[has_object_map]
                          for pred_boxes in pred_boxes_list]
        target_boxes_vec = target_boxes[has_object_map].view(-1, 4)

        # find the best boxes among the 2 (or self.B) predicted boxes and the corresponding iou
        best_ious, best_boxes = self.find_best_iou_boxes(
            pred_boxes_vec, target_boxes_vec)

        # compute regression loss between the found best bbox and GT bbox for all the cell containing objects
        reg_loss = self.get_regression_loss(
            best_boxes[..., :4], target_boxes_vec[..., :4]) * inv_N

        # compute contain_object_loss
        containing_obj_loss = self.get_contain_conf_loss(
            best_boxes[:, 4].unsqueeze(-1), torch.ones_like(best_ious)) * inv_N
