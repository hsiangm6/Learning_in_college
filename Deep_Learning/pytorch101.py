...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    rangeSTS=torch.arange(start, stop+1, dtype=torch.float64)
    # rangeSTS % 10 == 0 將生成一個對應的布林張量[False,...True,...False]
    x=rangeSTS[rangeSTS%10==0]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    M,N=x.shape
    #torch.flip(input,dim)逆序排列,ex:torch.flip(x, dims=[0])
    #arange(start,end,step,dtype)
    #unsqueeze(input,dim)在指定維度上增加一個維度
    #expand()複製原始張量，使其形狀與指定的大小匹配
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y[torch.arange(y.shape[1]-1), row_min_idxs]=0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    means=torch.sum(x, dim=0)/x.size(0)
    std = torch.sqrt(torch.sum((x - means)**2 / (x.size(0) - 1), dim=0))
    y=(x-means)/std
