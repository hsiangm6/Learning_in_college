...
## 懶人包：treatment的sample size就算沒有一致，一樣能使用TukeyHSD()(會自動偵測sample size)
...
#F = 9.2894, num df = 5, denom df = 994, p-value = 1.168e-08
aov.parent <- aov(reading.score ~ parental.level.of.education, data = stu_pf)
summary(aov.parent)

# Follow-up analysis
resultPractice <- TukeyHSD(aov.parent, conf.level=0.95)

## las=1 -> 使y軸的標題為橫向
plot(resultPractice, las=1, col="red", cex.axis=0.5)
